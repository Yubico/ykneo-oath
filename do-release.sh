...
if ! cat applet/src/pkgYkneoOath/YkneoOath.java | grep -q "VERSION = { `echo $VERSION | awk -F. '{print "0x0" $1 ", 0x0"$2 ", 0x0"$3}'`"; then
  echo "You need to update version in YkneoOath.java"
  exit
fi
...
releasename=ykneo-oath-${VERSION}
...
cd -
cp $releasedir/applet/bin/pkgYkneoOath/javacard/pkgYkneoOath.cap ${releasename}.cap
gpg --detach-sign --default-key $PGP_KEYID ${releasename}.tar.gz
gpg --detach-sign --default-key $PGP_KEYID ${releasename}.cap
$YUBICO_GITHUB_REPO/publish ykneo-oath $VERSION ${releasename}.tar.gz*
$YUBICO_GITHUB_REPO/publish ykneo-oath $VERSION ${releasename}.cap*
rm -rf $tmpdir
git push
git push --tags
